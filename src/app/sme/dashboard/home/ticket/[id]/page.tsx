...
export default function TicketDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (chatData && Array.isArray(chatData)) {
      const currentUserEmail = ticketData?.assignedSME;
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const currentUserEmail = ticketData?.assignedSME; 
    const receiverEmail = ticketData?.ticketCreatedBy;
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      <nav className="bg-white border-b sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <Link href='/sme/dashboard/home' className="flex items-center text-slate-500 hover:text-slate-800 transition-colors group">
