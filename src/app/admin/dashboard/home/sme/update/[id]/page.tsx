 ...
 * updateSME Component
 */
export default function UpdateSME({ params }: { params: Promise<{ id: number }> }) {
  ...
  const { data: smeDetails, isLoading: isDetailsLoading, error: detailsError } = useSmeDetails(id);
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      endDate: details.endDate ? String(details.endDate).split("T")[0] : "",
      ...
      whereDidYouFindOutAboutThisRole: details.whereDidYouFindOutAboutThisRole || "",
      previousEmployee:
        typeof details.previousEmployee === "boolean"
          ? (details.previousEmployee ? "Yes" : "No")
          : (details.previousEmployee || ""),
      nonDisclosureAgreement:
        typeof details.nonDisclosureAgreement === "boolean"
          ? (details.nonDisclosureAgreement ? "Yes" : "No")
          : (details.nonDisclosureAgreement || ""),
      relationStatus:
        typeof details.relationStatus === "boolean"
          ? (details.relationStatus ? "Yes" : "No")
          : (details.relationStatus || ""),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (resumeId) updatedData.resume = resumeId;
      if (coverLetterId) updatedData.coverLetter = coverLetterId;
      ...
      if (!res.ok) {
        ...
        const errorMessage = failedRes?.error?.message || failedRes?.error || "Update failed"
        const detailedErrors = failedRes?.error?.details?.errors
          ?.map((item: { message?: string }) => item?.message)
          ?.filter(Boolean)
          ?.join(", ");

        if (detailedErrors) {
          showToast(detailedErrors, "error")
      ...
      showToast('SME profile updated successfully!', "success")
      queryClient.invalidateQueries({ queryKey: ["smes"] });
      queryClient.invalidateQueries({ queryKey: ["smeDetails", id] });
      setLoading(false)
      router.push('/admin/dashboard/home/sme')
    }
    catch (err) {
      showToast('Something went wrong while updating this SME profile. Please try again later', "error")
      setLoading(false)
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const errorMessage = failedRes?.error?.message || failedRes?.error || "Delete failed";
        showToast(errorMessage, "error");
        return;
      }

      queryClient.invalidateQueries({ queryKey: ["smes"] });
      queryClient.removeQueries({ queryKey: ["smeDetails", id] });
      showToast("SME deleted successfully!", "success");
      setShowDeleteModal(false);
      router.push('/admin/dashboard/home/sme');
    } catch (err) {
      showToast("Something went wrong while deleting this SME.", "error");
    } finally {
      setDeleting(false);
    }
  };
  ...
  if (detailsError) {
    return <div className="p-6 text-red-500">Failed to load SME details</div>;
  }
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="px-6 py-8 sm:px-10 border-b border-slate-100 bg-slate-50/30">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-slate-900">Update SME profile</h2>
                <p className="text-sm text-slate-500">Update an existing profile in the expert database</p>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="px-6 py-8 sm:px-10 space-y-8">
                  ...
                  <div className="relative group border-2 border-dashed border-slate-200 rounded-xl p-4 hover:border-blue-400 transition-colors bg-slate-50/50">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="overflow-hidden">
                        <p className="text-xs font-semibold text-slate-700 truncate">{resume?.[0]?.name || "Select or drag file"}</p>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group border-2 border-dashed border-slate-200 rounded-xl p-4 hover:border-blue-400 transition-colors bg-slate-50/50">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="overflow-hidden">
                        <p className="text-xs font-semibold text-slate-700 truncate">{coverLetter?.[0]?.name || "Select or drag file"}</p>
                        <p className="text-[10px] text-slate-400">PDF, DOC up to 10MB</p>
                      </div>
                    </div>
                  </div>
            ...
            <div className="pt-8 flex flex-col-reverse sm:flex-row gap-4">
              <button
                ...
                type="button"
                disabled={deleting}
                className="flex-1 flex items-center justify-center gap-2 px-6 py-3 border border-red-200 text-red-700 rounded-xl hover:bg-red-50 transition-all font-semibold text-sm disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Trash2 className="w-4 h-4" />
                {deleting ? "Deleting..." : "Delete SME"}
              </button>
              <button
                onClick={() => router.back()}
                type="button"
